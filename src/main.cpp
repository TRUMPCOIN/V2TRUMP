...
const string strMessageMagic = "TrumpCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    {
        nSubsidy = 6500000 * COIN; //  TRUMP V1 SWAP
    }
        else if(pindexBest->nHeight < SAFETY_NET)
    {
        nSubsidy = 0 * COIN; // 10K EMPTY BLOCKS JUST IN CASE CHAIN GETS STUCK
    }
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProofOfWork() && nHeight > SAFETY_NET)
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "TrumpCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block        
        const char* pszTimestamp = "TrumpCoin V2 Main Network Live - Scrypt PoW - Blake PoS";
        CTransaction txNew;
        txNew.nTime = 1460362448;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1460362448;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 14585 : 0;

        
        
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x187872b69a077435c32369e51946755b7727789d5ed671ffa8fc960594bfcfc7"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xdf, 0xb4, 0xd4, 0x96 };
